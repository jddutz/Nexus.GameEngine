...
namespace Nexus.GameEngine.SourceGenerators
{
    ...
    public class PropertyBindingsGenerator : IIncrementalGenerator
    {
        ...
        static void Execute(SourceProductionContext context, ClassToGenerate? classToGenerate)
        {
            if (classToGenerate == null) return;
            ...
            {
                sb.AppendLine($"        public Nexus.GameEngine.Components.PropertyBinding? {prop} {{ get; init; }}");
            }
            sb.AppendLine();

            sb.AppendLine("        public override IEnumerator<(string propertyName, PropertyBinding binding)> GetEnumerator()");
            sb.AppendLine("        {");
