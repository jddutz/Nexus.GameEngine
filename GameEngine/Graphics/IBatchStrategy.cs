...
/// </summary>
public interface IBatchStrategy : IComparer<GLState>
{
    /// <summary>
    /// Computes a stable hash code for the specified <see cref="GLState"/> to facilitate efficient batch grouping.
    /// Render states with the same hash code are considered part of the same batch.
    /// </summary>
    /// <param name="state">The <see cref="GLState"/> to compute the hash code for.</param>
    /// <returns>An integer hash code representing the batchable aspects of the render state.</returns>
    int GetHashCode(GLState state);
